...
import { Card, Link, Value, Input } from "../../../../components/Index.jsx";
...
const CalculationFuelSpending = () => {
	...
	const calculate = () => {
		...
		axios.post("/api/geo/getDistance/byCity", {
			...
			setDistance(q.data.distance);
			setDuration(q.data.duration);
			setFuelConsumptionSum(((distance * 100).toFixed(2) * (fuelConsumption / 100).toFixed(2)).toFixed(2));
			setFuelPriceSumMin(((fuelConsumption / 100).toFixed(2) * fuelPriceMin).toFixed(2));
			setFuelPriceSumMax(((fuelConsumption / 100).toFixed(2) * fuelPriceMax).toFixed(2));
		});
		setResponse("ok");
	};
	...
	return <div className="page">
		<div className="content">
			...
			}
			{
				response === "ok"
					? <div>
						<Value title={t("distance")} value={distance}/>
						<Value title={t("duration")} value={duration}/>
						<Value title={t("fuelConsumptionSum")} value={fuelConsumptionSum}/>
						<Value title={t("fuelPriceSumMin")} value={fuelPriceSumMin}/>
						<Value title={t("fuelPriceSumMax")} value={fuelPriceSumMax}/>
					</div>
					: null
			}
		</div>
